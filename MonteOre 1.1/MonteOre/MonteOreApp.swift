...
struct NoteView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            // Header with project name and total time.
...
struct ProjectManagerView: View {
    ...
    // Generate share text using the actual text from the note columns.
    func generateShareText() -> String {
        var text = "Project\tTot Monte Ore\n"
        text += "----------------------------------------\n"
        for project in projectManager.projects {
            text += "\(project.name)\t\(project.totalProjectTimeString)\n"
            if project.noteRows.isEmpty {
                text += "  Nessuna nota\n"
            } else {
                text += "Giorno\tOrari\tTot Tempo\n"
...
struct ActivityView: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIActivityViewController {
        ...
        if let popover = controller.popoverPresentationController {
            // Fetch the key window from the current connected scenes (iOS 15+ compatible)
            if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
               let keyWindow = windowScene.windows.first(where: { $0.isKeyWindow }) {
                popover.sourceView = keyWindow
                popover.sourceRect = CGRect(x: keyWindow.bounds.midX,
                                            y: keyWindow.bounds.midY,
                                            width: 1,
                                            height: 1)
            } else {
                // Fallback: if no key window is found, use a zero rect from a new UIView
